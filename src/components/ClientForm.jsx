...
import { useState } from 'react';
import { TextField, Button, Container, Typography, Paper, Snackbar, Alert, Autocomplete } from '@mui/material';
import { useNavigate } from 'react-router-dom'; // Adicionado para redirecionamento
import useStore from '../store';
import { validateCPF, validateCNPJ } from '../validators';
import ArrowForwardIcon from '@mui/icons-material/ArrowForward';
...
const ClientForm = () => {
  ...
  // Lista de raças de cachorros e gatos
  const breeds = [
    'Labrador Retriever',
    'Golden Retriever',
    'Bulldog',
  ...
  const handleSubmit = async (e) => {
    ...
    // Cadastra o cliente
    const client = { name, cpfCnpj, phone, petName, breed };
    try {
      await addClient(client); // Usa a função addClient do store
      setName('');
      setCpfCnpj('');
